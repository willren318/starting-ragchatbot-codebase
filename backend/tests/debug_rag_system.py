...
class RAGSystemDebugger:
    ...
    def check_database_connectivity(self):
        ...
        try:
            ...
            if os.path.exists(self.config.CHROMA_PATH):
                self.log_success(
                    "DATABASE", f"Database directory exists: {self.config.CHROMA_PATH}"
                )
                ...
                self.log_warning(
                    "DATABASE",
                    f"Database directory does not exist: {self.config.CHROMA_PATH}",
                )
            ...
            try:
                ...
                if course_count > 0:
                    self.log_success(
                        "DATABASE", f"Found {course_count} courses in database"
                    )
                    ...
                    for course_meta in courses_metadata:
                        ...
                        self.log_success(
                            "DATABASE",
                            f"Course: {title} ({lesson_count} lessons, instructor: {instructor})",
                        )

                else:
                    self.log_issue(
                        "DATABASE",
                        "No courses found in database - this will cause query failures",
                    )
    ...
    def check_search_tools(self):
        ...
        try:
            ...
            if tool_def and "name" in tool_def:
                self.log_success(
                    "SEARCH_TOOL",
                    f"CourseSearchTool definition valid: {tool_def['name']}",
                )
            else:
            ...
            try:
                ...
                if "No relevant content found" in result:
                    self.log_success(
                        "SEARCH_TOOL",
                        "CourseSearchTool handles empty results correctly",
                    )
                elif "error" in result.lower():
                    self.log_issue(
                        "SEARCH_TOOL", f"CourseSearchTool returned error: {result}"
                    )
                else:
                    self.log_success(
                        "SEARCH_TOOL",
                        f"CourseSearchTool returned result: {result[:100]}...",
                    )
            ...
            if outline_def and "name" in outline_def:
                self.log_success(
                    "OUTLINE_TOOL",
                    f"CourseOutlineTool definition valid: {outline_def['name']}",
                )
            else:
                self.log_issue(
                    "OUTLINE_TOOL", "CourseOutlineTool definition is invalid"
                )
            ...
            try:
                ...
                if "No course found" in result:
                    self.log_success(
                        "OUTLINE_TOOL",
                        "CourseOutlineTool handles missing courses correctly",
                    )
                else:
                    self.log_success(
                        "OUTLINE_TOOL",
                        f"CourseOutlineTool returned result: {result[:100]}...",
                    )

            except Exception as e:
                self.log_issue(
                    "OUTLINE_TOOL", f"CourseOutlineTool execution failed: {e}"
                )
        ...
        try:
            ...
            if hasattr(ai_generator, "SYSTEM_PROMPT") and ai_generator.SYSTEM_PROMPT:
                ...
                if "Tool Usage" in prompt:
                    self.log_success(
                        "AI_GENERATOR", "System prompt includes tool usage guidance"
                    )
                else:
                    self.log_warning(
                        "AI_GENERATOR",
                        "System prompt may be missing tool usage guidance",
                    )

                if "Course Outline Tool" in prompt:
                    self.log_success(
                        "AI_GENERATOR",
                        "System prompt includes course outline tool guidance",
                    )
                else:
                    self.log_warning(
                        "AI_GENERATOR",
                        "System prompt may be missing course outline tool guidance",
                    )
            else:
        ...
        try:
            ...
            for tool_name in expected_tools:
                if tool_name in tool_names:
                    ...
                    self.log_issue(
                        "RAG_SYSTEM", f"Tool '{tool_name}' is not registered"
                    )
            ...
            try:
                ...
                if total_courses > 0:
                    self.log_success(
                        "RAG_SYSTEM", f"System has {total_courses} courses loaded"
                    )
                else:
                    self.log_issue(
                        "RAG_SYSTEM",
                        "No courses loaded - queries will fail with 'No relevant content found'",
                    )
    ...
    def simulate_query_flow(self, test_query: str = "What is Python programming?"):
        ...
        try:
            ...
            if analytics["total_courses"] == 0:
                self.log_warning(
                    "QUERY_SIMULATION",
                    "No courses loaded - query will return 'No relevant content found'",
                )
                ...
                try:
                    ...
                    with self._mock_ai_generator():
                        ...
                        self.log_success(
                            "QUERY_SIMULATION",
                            f"Query executed (mocked): '{response[:100]}...'",
                        )
                except Exception as e:
                    self.log_issue(
                        "QUERY_SIMULATION", f"Query failed even with mocked AI: {e}"
                    )
            else:
                self.log_success(
                    "QUERY_SIMULATION",
                    f"Ready to process queries with {analytics['total_courses']} courses",
                )

                # Note: We don't execute actual queries to avoid API costs
                self.log_success(
                    "QUERY_SIMULATION", "Query simulation would proceed normally"
                )
    ...
    def _mock_ai_generator(self):
        ...
        return patch.object(
            AIGenerator, "generate_response", side_effect=mock_generate_response
        )
    ...
    def run_comprehensive_check(self):
        ...
        warnings = [
            issue for issue in self.issues_found if issue["severity"] == "WARNING"
        ]
        ...
        if any(
            issue["component"] == "DATABASE" and "No courses found" in issue["issue"]
            for issue in self.issues_found
        ):
            print("💡 ROOT CAUSE IDENTIFIED: Empty database")
            print(
                "   SOLUTION: Load course documents using the document ingestion process"
            )
            print("   STEPS:")
            ...
            issue["component"] == "CONFIG" and "API_KEY" in issue["issue"]
            for issue in self.issues_found
        ):
            ...
            issue["component"] == "SEARCH_TOOL" and "execution failed" in issue["issue"]
            for issue in self.issues_found
        ):
            ...
            print(
                "   RECOMMENDATION: Run individual component tests for detailed analysis"
            )
