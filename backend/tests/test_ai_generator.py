...
class TestAIGenerator(unittest.TestCase):
    ...
    def test_tool_execution_error_handling(self):
        ...
        initial_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_tool", "input": {"query": "test"}, "id": "tool_1"}]
        )
        final_response = MockAnthropicResponse("I encountered an error while searching.")
    ...
    def test_sequential_tool_calling_two_rounds(self):
        ...
        round1_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "course X lesson 4"}, "id": "tool_1"}]
        )
        
        # Mock round 2: Tool use response  
        round2_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "Python Data Structures"}, "id": "tool_2"}]
        )
        
        # Mock final response after max rounds
        final_response = MockAnthropicResponse("Based on my searches, Course Y covers similar topics to Course X lesson 4.")
        ...
        # Verify final response
        self.assertEqual(response, "Based on my searches, Course Y covers similar topics to Course X lesson 4.")
    ...
    def test_sequential_tool_calling_early_termination(self):
        ...
        direct_response = MockAnthropicResponse("This is a general knowledge question. 2+2 equals 4.")
        ...
        self.assertEqual(response, "This is a general knowledge question. 2+2 equals 4.")
    ...
    def test_sequential_tool_calling_round1_tool_failure(self):
        ...
        round1_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "test"}, "id": "tool_1"}]
        )
    ...
    def test_sequential_tool_calling_round2_tool_failure(self):
        ...
        round1_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "first search"}, "id": "tool_1"}]
        )
        
        # Mock round 2: Tool use response
        round2_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "second search"}, "id": "tool_2"}]
        )
    ...
    def test_sequential_tool_calling_max_rounds_exceeded(self):
        ...
        round1_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "first"}, "id": "tool_1"}]
        )
        
        # Mock round 2: Tool use response (but this will hit max rounds)
        round2_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "second"}, "id": "tool_2"}]
        )
        
        # Mock final response without tools
        final_response = MockAnthropicResponse("Based on my research, here's the complete answer.")
    ...
    def test_sequential_tool_calling_context_preservation(self):
        ...
        round1_response = MockAnthropicResponse(
            stop_reason="tool_use", 
            tool_calls=[{"name": "search_course_content", "input": {"query": "basics"}, "id": "tool_1"}]
        )
        
        round2_response = MockAnthropicResponse(
            stop_reason="tool_use",
            tool_calls=[{"name": "search_course_content", "input": {"query": "advanced"}, "id": "tool_2"}] 
        )
        
        final_response = MockAnthropicResponse("Combined analysis of basics and advanced topics.")
...
class TestAIGeneratorRealAPI(unittest.TestCase):
    ...
    def test_tool_calling_with_mock_tools(self):
        ...
        mock_tool_manager = Mock()
        mock_tool_manager.execute_tool.return_value = "Mock course content about Python basics"
